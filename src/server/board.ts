...
export async function resetTicket(
  ...
  return withTransaction(db, async (client) => {
    ...
    if (rowCount === 0) {
      const fresh = await readTicket(client, ticketId)
      const freshSnap = fresh ? await readSnapshot(client, fresh) : snapshot
      throw new BoardError('CONFLICT', '作业票已被复位，终态不可逆', {
        snapshot: freshSnap,
        blockers: freshSnap.blockers,
      })
    }
